...
test('updateHintElement - recInfo is null', () => {
    ...
    assert.strictEqual(el.textContent, "Noch keine historischen Daten für diesen Inhaltstyp vorhanden. Standardempfehlung ist Qualität 60.");
...
test('updateHintElement - currentVal equals optimal', () => {
    ...
    assert.strictEqual(el.textContent, "Optimaler Wert für diesen Inhaltstyp basierend auf deiner Historie (Qualität 62).");
    assert.strictEqual(el.classList.contains("hidden"), false);
});
...
test('updateHintElement - currentVal is greater than optimal', () => {
    ...
    assert.strictEqual(el.textContent, "Deine Historie zeigt, dass dieser Inhaltstyp auch mit Qualität 58 ohne sichtbaren Qualitätsverlust gut komprimiert wird (spart mehr Platz).");
    assert.strictEqual(el.classList.contains("hidden"), false);
});
...
test('updateHintElement - currentVal is less than optimal', () => {
    ...
    assert.strictEqual(el.textContent, "Dieser Wert liegt unter dem empfohlenen Optimum von Qualität 65. Es könnte zu sichtbaren Kompressionsartefakten kommen.");
    assert.strictEqual(el.classList.contains("hidden"), false);
});
