...
function createMockElement() {
    ...
    return {
        ...
        dispatchEvent(event) {
            ...
            const propHandler = this['on' + event.type];
...
globalThis.bindNFOAgentEvents = () => {
    document.getElementById("btn-nfo-agent-done")?.addEventListener("click", () => {
        console.log("CLICKED DONE BUTTON", { wasFskOpen: wasFskModalOpenForNfoAgent, nfoAgentJobSuccess });
        const wasFskOpen = wasFskModalOpenForNfoAgent;
        closeNfoAgentModal();
        if (!wasFskOpen && typeof pollHealthStatus === "function") pollHealthStatus(true);
    });
    document.getElementById("btn-nfo-agent-cancel")?.addEventListener("click", () => {
...
test('Film-Rendering in FSK Modal uses media_kind', async () => {
    ...
    // Darf NICHT als Serie gerendert werden (obwohl hierarchy.show gesetzt ist), sondern muss in der Film-Sektion landen
...
test('NFO-Agent rendered in skipped_missing', async () => {
    ...
    assert.ok(html.includes("NFO Agent")); // Button vorhanden

    assert.ok(html.includes("/Filme/Fehlend")); // onclick agent_path vorhanden
...
test('Fertig-Klick bei ready=0 schliesst Modal', async () => {
    ...
    // Fetch requests for apply should be 0 because ready=0 triggers closeFskBatchModal
...
test('NFO Agent Lifecycle 4 states', async () => {
    ...
    // Mock wasFskModalOpenForNfoAgent
    ...
    assert.strictEqual(document.getElementById("fsk-batch-error-inline").style.display, "block");
    assert.strictEqual(document.getElementById("fsk-batch-error-inline").textContent, "NFO-Agent wurde abgebrochen."); // showFskBatchError is called with the message



    // Reset for missing job (no nfoAgentJobSuccess or errorMsg)
    modal.classList.add("active");
    modal.classList.remove("hidden");
    globalThis.wasFskModalOpenForNfoAgent = true;
    globalThis.nfoAgentJobSuccess = false;
    ...
    assert.strictEqual(document.getElementById("fsk-batch-error-inline").style.display, "none"); // No action, just goes back to preview
});
