...
class TestFSKBatchEndpoints(unittest.TestCase):
    ...
    def test_season_nfo_in_movie_folder(self, mock_load_settings):
        ...
        with open(os.path.join(movie_dir, "movie.nfo"), 'w') as f_nfo:

            f_nfo.write("<movie></movie>")

        season_nfo_path = os.path.join(movie_dir, "season.nfo")

        with open(season_nfo_path, 'w') as f_nfo:
        ...
        # Single scope applies to all NFOs in the folder.

        # But season.nfo is invalid here. The movie.nfo is valid though.

        # Wait, if we specifically request single scope on season.nfo path:

        res = self.client.post('/api/nas/fsk-batch/preview', json={

            "paths": [season_nfo_path], "scope": "single", "new_fsk": "12"

        })

        self.assertEqual(res.status_code, 400)

        self.assertIn("NFO-Datei ist unzulässig", res.json["message"])
    ...
    def test_same_count_race_condition(self, mock_load_settings, mock_write_fsk):
    ...
    def test_is_valid_series_root_anime(self, mock_load_settings):

        # Anime category without "serie" in its name

        mock_load_settings.return_value = {

            "nas_root": self.series_dir,

            "nas_paths": {"Anime": self.series_dir},

            "sync_categories": [{"name": "Anime", "nas_sub": "/", "type": "series"}]
        ...
        anime_dir = os.path.join(self.series_dir, "Ein Anime")

        os.makedirs(anime_dir, exist_ok=True)

        # Without season folder, just direct child of root

        self.assertTrue(is_valid_series_root(anime_dir, mock_load_settings.return_value))
