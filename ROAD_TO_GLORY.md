...
10. **Server-spezifische Artwork-Benennung durchgängig in der Pipeline:** (Erledigt) Auch außerhalb des reinen Metadaten-Downloads (z. B. beim Verschieben, Kopieren und YouTube-/Fallback-Pfaden in `processor.py`) müssen die Dateinamen für Poster/Fanart/Logo/Banner konsequent über `artwork_validators` für Emby/Jellyfin/Plex aufgelöst werden, damit keine festen `poster.jpg`/`fanart.jpg`-Annahmen mehr übrig bleiben.
...
### Phase 2: Sicherheit & Zugriffskontrolle (Erledigt)

#### 2.1 Authentifizierung (LAN-Sicherheit) - Umgesetzt
- Optionale Passwort- oder PIN-Sperre im Backend (gekoppelt an das Vorhandensein von `password_hash` in `settings.json`).
