...
class HealthCacheManager:
    ...
    def calculate_hybrid_state(self, folder_path: str, validator, is_movie: bool) -> dict:
        ...
        if is_movie:
            ...
            # 1.1 Rekursiv alle Videos und NFOs erfassen, um Episoden-Änderungen aufzudecken
            video_files = {}
            nfo_files = {}
            video_extensions = {'.mkv', '.mp4', '.avi', '.webm', '.mov', '.ts', '.m2ts'}
            ...
            try:
                for root, dirs, files in os.walk(abs_path):
                    for f in files:
                        ...
                        rel_p = os.path.relpath(full_path, abs_path)
                        if ext in video_extensions:
                            try:
                                video_files[rel_p] = [os.path.getmtime(full_path), os.path.getsize(full_path)]
                            except Exception:
                                pass
                        elif ext == '.nfo':
                            try:
                                nfo_files[rel_p] = os.path.getmtime(full_path)
                            except Exception:
                                pass
            except Exception:
                pass
