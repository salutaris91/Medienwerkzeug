...
def apply_renames(target_folder, rename_plan):
    ...
    for item in rename_plan:
        ...
        new_rel_sanitized = os.path.join(parent_dir, sanitize_filename(filename))

        new_rel_lower = new_rel_sanitized.lower()
        if new_rel_lower in target_paths:
            return {"status": "error", "message": f"Kollision im Umbenennungsplan: Mehrere Dateien sollen zu '{new_rel_sanitized}' umbenannt werden."}
        target_paths.add(new_rel_lower)
        item["proposed_rel_path"] = new_rel_sanitized
